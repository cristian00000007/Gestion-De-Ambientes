...
<script>
    function borrar(id){
                swal({
                    ...
                    text: "El registro eliminado no se pude recuperar",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                    })
                    .then((willDelete) => {
                    if (willDelete) {
                        var url2='{{ route('eliminarAm','id') }}';
                            url2 = url2.replace('id',id);
                        location.href=url2;
                    } else {
                        swal("El registro no se va a borrar");
                    }
                    });
  ...
  if (dt_basic_table.length) {
    var dt_basic = dt_basic_table.DataTable({
      ...
      columns: [
                { data: 'id'},
                { data: 'Ambien_Nombres'},
                { data: 'Ambien_Capacidad'},
                { data: 'Ambien_Observacion'},
                { data: 'Ambien_Estado'},
                { data: 'CategoriaAmbienteId'},
                { data: 'Accions'}
            ],
      columnDefs: [
        {
    ...
    $('div.head-label').html('<h6 class="mb-0">Listado de Ambientes</h6>');
  }
