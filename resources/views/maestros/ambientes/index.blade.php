            ...
            <li class="nav-item dropdown dropdown-user"><a class="nav-link dropdown-toggle dropdown-user-link" id="dropdown-user" href="#" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                ...
                <div class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdown-user" onclick="ver()">
                    ...
                    <div id="perfil">
                        ...
                        <a id="perfil" class="dropdown-item" href="{{ route('perfilA') }}"><i class="me-50" data-feather="user"></i> Profile</a>
                    </div>
...
<!-- END: Main Menu-->
                ...
                <section id="basic-input" class="tooltip-validations">
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <input class="form-control" style="width: 20rem; float: right; background: #fdfdfd;" type="text" name="buscar" id="buscar" placeholder="Buscar Ambiente">
                            <br><br><br><br>
                            <form method="POST" action="{{ route('programacionAm') }}" >
                                @csrf
                                <div class="row">
                                    {{-- @foreach ($verProg as $info) --}}

                                    @foreach ($verAm as $ca)
                                    {{-- @if ($info->ambientesIdAmbientes == $ca->id) --}}
                                    <div class="col-md-4" id="fo{{ $ca->id }}">
                                        ...
                                        <div class="card text-center" style="width: 20rem; background: #fafafa;">
                                            <div class="card-body">
                                                ...
                                                <div class="card">
                                                    ...
                                                    <img class="card-img-top" src="{{ asset('storage/'.$ca->Ambien_Foto) }}" alt="Card image cap">
                                                    <div class="card-body">
                                                      <h4 class="card-title">{{ $ca->Ambien_Nombres }}</h4>
                                                      <p class="card-text">
                                                      </p>


                                                      {{-- <button type="submit" id="boton" class="btn btn-primary" name="id" value="{{ $info->id }}">Entrar</button> --}}
                                                    <div class="card-footer" id="ju{{ $ca->id }}">
                                                        <small>Disponible</small>
                                                    </div>
                                                    <button type="submit" id="boton" class="btn btn-primary" name="id" value="{{ $ca->id }}">Entrar</button>
                                                    </div>
    ...
    <input type="hidden" id="uu" value="{{ $tiempo }}">
@include('layouts.Footer')
@foreach ($verProg as $info)
@foreach ($verAm as $ca)
<script>
@if ($info->ambientesIdAmbientes == $ca->id)
 @if ($tiempo == $info->Program_Franja)
 $('#ju{{ $ca->id }}').html('<small>Ocupado</small>');
 @endif
 @if ($tiempo != $info->Program_Franja)
 $('#ju{{ $ca->id }}').html('<small>Disponible</small>');
 @endif
@endif
// @if ($info->ambientesIdAmbientes != $ca->id)
// $('#ju{{ $ca->id }}').html('<small>Disponible</small>');
...
<script>
    document.addEventListener("keyup", e=>{
        if(e.target.matches('#buscar')){
            ...
            document.querySelectorAll('#fo{{ $ca->id }}').forEach(ambiente => {
                ambiente.textContent.toLowerCase().includes(e.target.value.toLowerCase())
                ?ambiente.classList.remove('filtro')
                :ambiente.classList.add('filtro')
            })
        }
    })
</script>
