...
<script>
    function borrar(id){
                swal({
                    ...
                    .then((willDelete) => {
                    if (willDelete) {
                        var url2='{{ route('eliminarPcion','id') }}';
...
$(function () {
  ...
  if (dt_basic_table.length) {
    var dt_basic = dt_basic_table.DataTable({
        ajax: '{{ route('listPcion') }}',
      //ajax: 'http://localhost/Laravel%20Pro%20Amb/public/programas/listjson',
      columns: [
            { data: 'id' },
            { data: 'Program_Fecha'},
            { data: 'Program_Trimestre'},
            { data: 'Program_Franja'},
            { data: 'ambientesIdAmbientes'},
            { data: 'FichaId'},
            { data: 'InstructorId'},
            { data: 'Acciones'},
          ],
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            var id = full['id'];
            var url = '{{ route('actualizarPcion','id') }}';
    ...
    $('div.head-label').html('<h6 class="mb-0">Listado de Programaciones</h6>');
  }
